...
rho = args[4] #as.numeric(args[4]) # the parameter for LD
...
fn.result <- function(x){
 p <- Get_inter_glmm(data[[x]], alpha=0.1, pow = c(1:6, Inf), n.perm =2000, 
...
################################################################
p_aGE <- p_IndaGE <- p_waGE <- matrix(NA, ncol=9, nrow = M)
...
#time <- cbind(time.aGE.inter, time.IndaGE.inter, time.waGE.inter, time.rareGE.inter, time.IndrareGE.inter, time.wrareGE.inter)
...
#write.table(time, file='time_inter_test.txt', quote=FALSE,append=FALSE) 
#write.table(phi, file='phi_value.txt', quote=FALSE,append=FALSE) 
