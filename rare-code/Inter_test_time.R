...
dir0 <- paste0("~/Rare-variant", "/", "Inter_test","/", variant)
setwd(dir0)
set.seed(1234)
...
######### aGE, IndaGE, and waGE ####################
fn.aGE <- function(x){
  p.aGE <- aGE.inter(data[[x]], pow = c(1:6, Inf), n.perm=2000, 
...
fn.IndaGE <- function(x){
  p.IndaGE <- IndaGE.inter(data[[x]], pow = c(1:6, Inf), n.perm=2000, 
                           mainweights=weights, interweights=weights)
  return(p.IndaGE)
}
...
p_aGE <- p_IndaGE <- matrix(NA, ncol=9, nrow = M)
...
fn.waGE <- function(x){
  p.waGE <- WaGE.inter(data[[x]], p_aGE[x, ], p_IndaGE[x, ], n.perm=2000, alpha=0.1)
  return(p.waGE)
}
mem_waGE <- rep(NA, M)
for(i in 1:M){
...
fn.rareGE <- function(x){
  p.rareGE <- rareGE.inter(data[[x]], n.perm=2000, 
                           mainweights=weights, interweights=weights)
  return(p.rareGE)
}
...
fn.IndrareGE <- function(x){
  p.IndrareGE <- IndrareGE.inter(data[[x]], n.perm =2000, 
                                 mainweights=weights, interweights=weights)
  return(p.IndrareGE)
}
...
fn.wrareGE <- function(x){
  p.wrareGE <- WrareGE.inter(data[[x]], p_rareGE[x], p_IndrareGE[x], n.perm=2000, alpha=0.1)
  return(p.wrareGE)
}
...
name <- c(c(paste("aGEsm", pow = c(1:6, Inf), sep = ""), "aGEsm", "aGEsm_fisher"), 
          c(paste("IndaGEsm", pow = c(1:6, Inf), sep = ""), "IndaGEsm", "IndaGEsm_fisher"), 
          c(paste("waGEsm", pow = c(1:6, Inf), sep = ""), "waGEsm","waGEsm_fisher"), 
...
write.table(power, file='power_inter_test.txt',  quote=FALSE,append=FALSE)
write.table(p_value, file='Pvalue_inter_test.txt', quote=FALSE,append=FALSE) ### For ROC and AUC
write.table(time.inter, file='time_inter.txt', quote=FALSE,append=FALSE) 
